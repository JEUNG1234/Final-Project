...
import defaultImage from '../../assets/돌하르방.jpg';
...
const WorkcationEnrollForm = () => {
  ...
  // 파일 인풋 참조를 위한 useRef 훅
  const fileInputRef = useRef(null);

  // 이미지 등록 버튼 클릭 시 실제 파일 인풋 클릭 트리거
  ...
  const handleFileChange = (event) => {
    ...
    if (selectedFile) {
      ...
      reader.onloadend = () => {
        setPreviewImageUrl(reader.result); // <--- 미리보기 URL을 state에 저장
      };
      reader.readAsDataURL(selectedFile); // 파일을 Data URL로 읽기
      // 여기에 파일 업로드 로직 추가 (예: FormData를 사용하여 서버로 전송)
    } else {
      setSelectedImageName(''); // 파일 선택이 취소되면 Input 필드를 비웁니다.
      setPreviewImageUrl(defaultImage); // <--- 파일 선택 취소 시 기본 이미지로 되돌리기
    }
  };
    ...
    <FullWapper>
      <MainContent>
        ...
        {activeTab === 'intro' && (
          <>
            <ImageSection>
              <img src={previewImageUrl} alt="장소 이미지" />
            </ImageSection>{' '}
            {/* 실제 이미지 URL로 교체 필요 */}
      ...
      <DateContent>
        ...
        {activeTab === 'intro' && (
          <>
            ...
            <FormGroup>
              <Label htmlFor="imageName">이미지</Label>
              <Input id="imageName" type="text" placeholder="이미지명" value={selectedImageName} readOnly />
              <HiddenFileInput
                id="fileUpload" // HiddenFileInput과 연결될 ID
                type="file"
                ref={fileInputRef}
                onChange={handleFileChange}
              />
              <StyledUploadButton htmlFor="fileUpload">이미지등록</StyledUploadButton>
            </FormGroup>
            ...
            <ActionButtons>
              <DangerButton>취소하기</DangerButton>
        ...
        {activeTab === 'precautions' && (
          <>
            ...
            <FormGroup>
              <Label htmlFor="imageName">이미지</Label>
              <Input id="imageName" type="text" placeholder="유의사항 이미지" value={selectedImageName} readOnly />
              <HiddenFileInput
                id="fileUpload" // HiddenFileInput과 연결될 ID
                type="file"
                ref={fileInputRef}
        ...
        <Modal isOpen={isMapModalOpen} onClose={handleCloseMapModal} title="장소 주소 검색">
