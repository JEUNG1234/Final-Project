    ...
    <MainContent>
      ...
      <SliderWrapper>
        <ContentBody>
          ...
          <SliderWithLabels
            question="1. 나는 내 감정을 스스로 잘 조절할 수 있다고 느낀다."
            min={1}
            max={10}
            initialValue={5}
          />
          <SliderWithLabels question="2. 최근 들어 우울하거나 무기력한 기분을 자주 느낀다." min={1} max={10} />
          <SliderWithLabels question="3. 나는 내 삶에 만족감을 느낀다." min={1} max={10} />
          <SliderWithLabels question="4. 사소한 일에도 쉽게 불안하거나 긴장하게 된다." min={1} max={10} />
          <SliderWithLabels question="5. 다른 사람과의 관계에서 편안함을 느낀다." min={1} max={10} />
        </ContentBody>

        <ContentBody>
          <SliderWithLabels question="6. 나 자신에 대한 신뢰와 자존감이 있다." min={1} max={10} />
          <SliderWithLabels question="7. 최근에 혼자 있고 싶다는 생각을 자주 한다." min={1} max={10} />
          <SliderWithLabels question="8. 나는 미래에 대한 희망이나 기대감을 가지고 있다." min={1} max={10} />
          <SliderWithLabels question="9. 감정이 자주 요동치고 이유 없이 기분이 다운될 때가 있다." min={1} max={10} />
          <SliderWithLabels question="10. 일상에서 즐거움을 느끼는 일이 자주 있다." min={1} max={10} />
        </ContentBody>
      </SliderWrapper>

      <ButtonGroup>
        <PageButton onClick={() => navigate('/healthcaremain')}>뒤로가기</PageButton>
        <PageButton onClick={() => navigate('/mentalcareresult')}>결과 확인하기</PageButton>
      </ButtonGroup>
    </MainContent>
  );
};
